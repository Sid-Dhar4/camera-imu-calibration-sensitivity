...
if [ "$#" -lt 1 ] || [ "$#" -gt 2 ]; then
  echo "Usage: bash scripts/run_openvins_prepared_run_generic.sh <run_id> [nominal|perturbed]" >&2
  exit 2
fi

RUN_ID="$1"
RUN_GROUP="${2:-perturbed}"
...
TRAJ_DIR="$REPO/results/trajectories/${RUN_GROUP}/${RUN_ID}"
...
printf "Log: %s\n\n" "$LOG_PATH"
printf "Leave this running until bag playback ends, then press Ctrl+C once here.\n\n"
