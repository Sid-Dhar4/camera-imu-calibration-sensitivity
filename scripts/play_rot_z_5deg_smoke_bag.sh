...
LOG_PATH="$HOME/robotics_ws/camera-imu-calibration-sensitivity/results/logs/perturbed/${RUN_ID}/ros2_bag_play_rot_z_5deg_smoke.log"

mkdir -p "$(dirname "$LOG_PATH")"

printf "\n=== Terminal 3: play MH_01_easy for rotation_z_5deg smoke ===\n"
printf "Run ID: %s\n" "$RUN_ID"
printf "Bag: %s\n" "$BAG"
printf "Log: %s\n\n" "$LOG_PATH"
