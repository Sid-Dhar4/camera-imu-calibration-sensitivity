...
LOG_PATH="$HOME/robotics_ws/camera-imu-calibration-sensitivity/results/logs/perturbed/${RUN_ID}/ros2_bag_play_rot_z_5deg_full.log"

mkdir -p "$(dirname "$LOG_PATH")"

printf "\n=== Terminal 3: play full MH_01_easy bag for rotation_z_5deg ===\n"
printf "Run ID: %s\n" "$RUN_ID"
printf "Bag: %s\n" "$BAG"
printf "Log: %s\n\n" "$LOG_PATH"
