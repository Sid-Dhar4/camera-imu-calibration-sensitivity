...
def build_summary(rows: list[dict]) -> list[dict]:
    specs = [
        ("nominal", "none", "0", "nominal"),
        ("rotation", "z", "2.0", "rotation_z_2deg"),
        ("rotation", "z", "5.0", "rotation_z_5deg"),
    ]
    summary = []
    for perturb_type, axis, magnitude, label in specs:
...
def write_plot(summary: list[dict], path: Path) -> None:
    ...
    plt.title("MH_03_medium rotation-z calibration sensitivity")
    plt.tight_layout()
    plt.savefig(path, dpi=180)


def write_report(summary: list[dict], path: Path) -> None:
    nom, rot2, rot5 = summary
    report = f"""# MH_03_medium Rotation-z Sensitivity
...
|---|---:|---:|---:|---:|
| nominal | {nom["ate_rmse_m"]:.6f} | {nom["ate_increase_x"]:.2f}x | {nom["rpe_trans_rmse_m"]:.6f} | {nom["rpe_rot_rmse_deg"]:.6f} |
...
The 2 degree rotation-z perturbation increases ATE RMSE from `{nom["ate_rmse_m"]:.6f} m` to `{rot2["ate_rmse_m"]:.6f} m`, a `{rot2["ate_increase_x"]:.2f}x` increase.

The 5 degree rotation-z perturbation increases ATE RMSE to `{rot5["ate_rmse_m"]:.6f} m`, a `{rot5["ate_increase_x"]:.2f}x` increase. This indicates severe trajectory divergence even though OpenVINS produced an output trajectory.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Summarize MH03 rotation-z calibration sensitivity.")
    parser.add_argument("--metrics", default="results/metrics.csv")
    parser.add_argument("--table", default="results/tables/mh03_rotation_z_sensitivity.csv")
    parser.add_argument("--plot", default="results/plots/mh03_rotation_z_sensitivity_ate_rmse.png")
    parser.add_argument("--report", default="reports/mh03_rotation_z_sensitivity.md")
