...
CONFIG_PATH="$HOME/robotics_ws/camera-imu-calibration-sensitivity/results/configs_used/${RUN_ID}/estimator_config_smoke_safe.yaml"
TRAJ_DIR="$HOME/robotics_ws/camera-imu-calibration-sensitivity/results/trajectories/nominal/${RUN_ID}"
LOG_PATH="$HOME/robotics_ws/camera-imu-calibration-sensitivity/results/logs/nominal/${RUN_ID}/openvins_direct_smoke.log"

mkdir -p "$TRAJ_DIR"
rm -f "$TRAJ_DIR/ov_estimate.txt" "$TRAJ_DIR/ov_estimate_std.txt" "$TRAJ_DIR/ov_groundtruth.txt" "$TRAJ_DIR/traj_timing.txt"

printf "\n=== Terminal 2: OpenVINS direct nominal smoke node ===\n"
printf "Config: %s\n" "$CONFIG_PATH"
